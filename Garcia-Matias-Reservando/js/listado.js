...
Listado.prototype.buscarRestaurante = function(id) {
    let elem = this.restaurantes.find((elem) => elem.id === id);
    if (elem) {
        return elem;
    }
...
//Funcion General para filtrar Rubros, Horarios, Ubicaciones
var filtrados = (a) => a.filter((elem, index, self) => {
...
Listado.prototype.obtenerRubros = function() {
    var rubro = this.restaurantes.map((elem) => {
        return elem.rubro;
    });

    var rubro2 = filtrados(rubro);
...
Listado.prototype.obtenerHorarios = function() {
    ...
    var arregloHorarios = this.restaurantes.map((elem) => {
